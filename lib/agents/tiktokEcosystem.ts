...
//
// Uses FILE_UPLOAD as the source, not PULL_FROM_URL: PULL_FROM_URL requires
// verifying domain ownership of the media host inside TikTok's developer
// portal first, which is extra setup outside this codebase. FILE_UPLOAD
// needs no domain verification -- we fetch the video bytes ourselves and
// PUT them straight to TikTok, so it works the moment OAuth is connected.
